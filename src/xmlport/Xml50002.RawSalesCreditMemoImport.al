xmlport 50002 "Raw Sales Credit Memo Import"
{
    // Pallavi 24012017
    // created xmlport for rawsales credit memo data in update log
    ...
    // prdp 31012017
    // added new field comments
    // prdp 02022017
    // set min occur property to zero for field Comments
    // prdp 300317
    // modified code for case ID
    // Pradip 110412017
    // added code for validaitng policy no canceled status using policy master - ValDATErAWCREDITMEMO()
    // added code for updating policy maste alongiwth doc umenr servicemaster = updateservicemaster()
    // prdp 120417
    // added code to delete * from policy number
    // prdp 23062017
    ...
    begin
        // for vladiating raw data
        //UpdateRawData.ValidateRawDataCreditMemo(CASIDTEXT);

        ValidateRawDataCreditMemo(CASIDTEXT);
    end;
    ...
    var
        ...
        XMLRejectMAster: XMLport "Sales Cr Memo Error Export";
        ...
        RawSalesCreditMemoImport: XMLport "Raw Sales Credit Memo Import";
        PolNo: Code[50];
        DepDimNotExist: Label 'DepDimNotExist';
        BusLineDimNotExist: Label 'BusLineDimNotExist';
        GLINVALID: Label 'GL No is Invalid';
        GLAcc: Record "G/L Account";
        COMMENSTBLANK: Label 'Comments must not be blank';
        ...
        Text002: Label 'Invoice No. %1  has only %2  as remaining amount but credit memo amount is %3';
        DimSetEntry: Record "Dimension Set Entry";
        SalesHdr: Record "Sales Header";
        DefTemp: Record "Deferral Template";
        SalesLine1: Record "Sales Line";
        EvaluatedInvoiceDate: Date;
        Evaluated_Posting_No: Code[250];
            ...
            REPEAT
                ...
                */
                ...
                IF RawUpdateLog.PolicyNo_OEMInv_RefNo <> '' THEN BEGIN
                    IF NOT PolicyMaster.GET(RawUpdateLog.PolicyNo_OEMInv_RefNo) THEN BEGIN
                        PolicyMaster.INIT;
                        PolicyMaster.PolicyNo := RawUpdateLog.PolicyNo_OEMInv_RefNo;
                        PolicyMaster.INSERT;
                    END
                    ELSE BEGIN
                        RawUpdateLog."Error description" := RawUpdateLog."Error description" + ' ; ' + POLICYEXIST;
                ...
                IF RawUpdateLog."Vehicle Identification No" <> '' THEN BEGIN
                    IF NOT VINMaster.GET(RawUpdateLog."Vehicle Identification No") THEN BEGIN
                        VINMaster.INIT;
                ...
                IF RawUpdateLog.Comments = '' THEN BEGIN
                    RawUpdateLog."Error description" := RawUpdateLog."Error description" + ' ; ' + COMMENSTBLANK;
                    FoundError := TRUE;
                END;
                //END;
            ...
            REPEAT
                ...
                IF insert1 = 1 THEN BEGIN
                    ...
                    IF Sales_HDRModify.FINDFIRST THEN BEGIN
                        ...
                        Sales_HDRModify."Posting No. Series" := 'SINV1';
    ...
    begin
        ...
        IF RawUpdateLog.FINDSET THEN
            REPEAT
                ...
                PolNo := DELCHR(RawUpdateLog.PolicyNo_OEMInv_RefNo, '=', '*');
                ...
                IF PolNo <> '' THEN BEGIN
                    PolicyMaster.RESET;
                    PolicyMaster.SETRANGE(CustNo, RawUpdateLog.Sell_To_Customer_No);
                    PolicyMaster.SETRANGE("Policy Start Date", RawUpdateLog.Policy_Coverage_Start_Date);
                    PolicyMaster.SETRANGE("Policy End Date", RawUpdateLog.Policy_Coverage_End_Date);
                    PolicyMaster.SETRANGE(PolicyNo, PolNo);
                    IF NOT PolicyMaster.FINDFIRST THEN BEGIN
                        RawUpdateLog."Error description" := RawUpdateLog."Error description" + ' ; ' + POLICYNOTEXIST;
                        FoundError := TRUE;
                    END
                    ELSE BEGIN
                        IF PolicyMaster.Cancelled = TRUE THEN BEGIN
                ...
                IF RawUpdateLog."Department Dim" <> '' THEN BEGIN
                    ...
                    IF NOT DimVal.FINDFIRST THEN BEGIN
                        RawUpdateLog."Error description" := RawUpdateLog."Error description" + ' ; ' + 'DepCodeDoesNotExist';
                        FoundError := TRUE;
                    END;
                END;
                ...
                IF RawUpdateLog."Business Line Dim" <> '' THEN BEGIN
                    ...
                    IF NOT DimVal.FINDFIRST THEN BEGIN
                        RawUpdateLog."Error description" := RawUpdateLog."Error description" + ' ; ' + 'BusinessLineCodeDoesNotExist';
                        FoundError := TRUE;
                    END;
                END;
                ...
                IF RawUpdateLog."Reference Invoice Number" = '' THEN BEGIN
                    RawUpdateLog."Error description" := RawUpdateLog."Error description" + ' ; ' + 'ReferenceInvNoMandatory';
                    FoundError := TRUE;
                END ELSE BEGIN
                    CustLedEntry.RESET;
                    CustLedEntry.SETRANGE(CustLedEntry."Document Type", CustLedEntry."Document Type"::Invoice);
                    CustLedEntry.SETRANGE("Document No.", RawUpdateLog."Reference Invoice Number");
                    CustLedEntry.SETRANGE(Open, TRUE);
                    IF NOT CustLedEntry.FINDFIRST THEN BEGIN
                        RawUpdateLog."Error description" := RawUpdateLog."Error description" + ' ; ' + 'InvalidReferenceInvNo';
                        FoundError := TRUE
                    END;
                END;
            ...
            REPEAT
                ...
                IF insert1 = 1 THEN BEGIN
                    ...
                    IF Sales_HDRModify.FINDFIRST THEN BEGIN
                        ...
                        //Sales_HDRModify.VALIDATE("Posting No.",RawDataLog.PostingNo); //CCIT-04082022
                        //CCIT-12082022
                        ...
                        IF RawDataLog."Reference Invoice Number" <> '' THEN BEGIN
                            Sales_HDRModify.VALIDATE(Sales_HDRModify."Applies-to Doc. Type", Sales_HDRModify."Applies-to Doc. Type"::Invoice);
                            Sales_HDRModify.VALIDATE("Applies-to Doc. No.", RawDataLog."Reference Invoice Number");
                            Sales_HDRModify.VALIDATE("Reference Invoice Number", RawDataLog."Reference Invoice Number");
                        END;
                ...
                IF LineInsert = 1 THEN BEGIN
                    ...
                    IF RecSalesLine.FINDFIRST THEN BEGIN
                        ...
                        */
                        ...
                        SalesHdr.RESET;
                        SalesHdr.SETRANGE(SalesHdr."Document Type", SalesHdr."Document Type"::"Credit Memo");
                        ...
                        //CCIT-07012022
                        IF RawDataLog."Ship To Code" <> '' THEN BEGIN
                            IF RawDataLog."Ship To Code" = 'FG-27-MAH' THEN
                                RecSalesLine.VALIDATE("GST Place of Supply", 1)
                            ELSE
                                RecSalesLine.VALIDATE("GST Place of Supply", 2);
                        END;
                        //CCIT-07012022
                        ...
                        Shdr.RESET;
                        Shdr.SETRANGE(Shdr."Document Type", Shdr."Document Type"::"Credit Memo");
                        Shdr.SETRANGE("No.", Document_No2);
                        IF Shdr.FINDFIRST THEN BEGIN


                            // IF Shdr.Structure <> '' THEN BEGIN
                            //   SalesLine1.CalculateStructures(Shdr);
                            //   SalesLine1.AdjustStructureAmounts(Shdr);
                            //   SalesLine1.UpdateSalesLines(Shdr);
                            //   SalesLine1.CalculateTCS(Shdr);
                            //   COMMIT;
                            // END ELSE BEGIN
                            //   SalesLine1.CalculateTCS(Shdr);
                            //   COMMIT;
                            ...
                            Shdr.CALCFIELDS(Amount);

                            CLEAR(recCustLedgEntry);
                            recCustLedgEntry.SETRANGE(recCustLedgEntry."Document No.", RawDataLog."Reference Invoice Number");
                            IF recCustLedgEntry.FINDFIRST THEN BEGIN
                                decAmountToApply := ABS(Shdr.Amount);
                                ...
                                IF ABS(recCustLedgEntry."Amount to Apply") > ABS(recCustLedgEntry."Remaining Amt. (LCY)") THEN
                                    ERROR(Text002, recCustLedgEntry."Document No.", recCustLedgEntry."Remaining Amt. (LCY)", recCustLedgEntry."Amount to Apply");
    ...
    begin
        ...
        RawDataLog.RESET;
        RawDataLog.SETRANGE(CaseID, CASEID);
        IF RawDataLog.FINDSET THEN
            REPEAT
                ...
                PolicyNo := DELCHR(RawDataLog.PolicyNo_OEMInv_RefNo, '=', '*');
                // for cancelling status
                DocServMaster.RESET;
                DocServMaster.SETRANGE(DocServMaster."Policy No/OEM Inv/Ref No", PolicyNo);
                DocServMaster.SETRANGE(DocServMaster."Policy / Coverage Start Date", Evaluated_Policy_Coverage_Start_Date);
                DocServMaster.SETRANGE(DocServMaster."Policy / Coverage End Date", Evaluated_Policy_Coverage_End_Date);
                DocServMaster.SETRANGE("NAV Customer", RawDataLog.Sell_To_Customer_No);
                IF DocServMaster.FINDFIRST THEN BEGIN
                ...
                // for modifying policy master
                PolicyMaster.RESET;
                PolicyMaster.SETRANGE(CustNo, RawDataLog.Sell_To_Customer_No);
                PolicyMaster.SETRANGE("Policy Start Date", RawDataLog.Policy_Coverage_Start_Date);
                PolicyMaster.SETRANGE("Policy End Date", RawDataLog.Policy_Coverage_End_Date);
                PolicyMaster.SETRANGE(PolicyNo, RawDataLog.PolicyNo_OEMInv_RefNo);
                IF PolicyMaster.FINDFIRST THEN BEGIN
                    PolicyMaster.Cancelled := TRUE;
                    PolicyMaster.MODIFY;
                END;
