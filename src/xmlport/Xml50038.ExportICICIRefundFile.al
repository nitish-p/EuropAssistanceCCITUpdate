XmlPort 50038 "Export ICICI Refund File"
{
    FieldDelimiter = '<None>';
    FieldSeparator = '|';
    ...
    schema
    {
        textelement(Root)
        {
            ...
            tableelement("Bank Account Ledger Entry";"Bank Account Ledger Entry")
            {
                ...
                SourceTableView = sorting("Entry No.") order(ascending) where("Document Type"=const(6),"Suggsted Payment"=const(true),"Bank Account No."=const('BANK0001'));
                textelement(DebitAcNo)
                {
                }
                fieldelement(BeneficiaryAcNo;"Bank Account Ledger Entry"."Beneficiary Acc. No.")
                {
                }
                fieldelement(BeneficiaryName;"Bank Account Ledger Entry"."Beneficiary Name")
                {
                }
                ...
                fieldelement(IFSC;"Bank Account Ledger Entry"."Beneficiary IFSC Code")
                {
                }
                ...
                begin
                    ...
                    AddDetails5:="Bank Account Ledger Entry"."Bal. Account No.";
                    recCustomer.Reset;
                    recCustomer.SetRange("No.","Bank Account Ledger Entry"."Bal. Account No.");
                    if recCustomer.FindFirst then begin
                    Remarks:=recCustomer.Name;
                    end;
                end;
            }
        }
