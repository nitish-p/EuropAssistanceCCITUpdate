Report 50080 GSTB2BSales
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/GSTB2BSales.rdl';
    UsageCategory = Lists;
    ApplicationArea = all;
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Document No.", "Document Line No.", "GST Component Code") order(ascending) where("Transaction Type" = const(Sales), "GST Customer Type" = const(Registered), "Source Type" = const(Customer), "Entry Type" = const("Initial Entry"), "Document Type" = const(Invoice));
            ...
            column(AmtToCust; '')//"Detailed GST Ledger Entry"."Amount to Customer/Vendor"
            {
            }
            column(Invoice_Type; '')//"Detailed GST Ledger Entry"."Invoice Type"
            {
            }
            //Naveen
