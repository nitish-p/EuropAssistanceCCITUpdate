Report 50038 "SFTP Audit"
{
    ...
    begin
        ...
        JobQueueEntry.Reset;
        JobQueueEntry.SetRange("Object ID to Run", 50038);
        if JobQueueEntry.FindFirst then begin
            ...
            AuditFileName := JobQueueEntry."Expected Audit File";

            Vend.Reset;
            Vend.SetRange("Audit File Name", AuditFileName);
            if not Vend.FindFirst then begin
                DownloadFile(AuditFileName);
            end;
        end;
    ...
    begin
        ...
        AuditFileName := 'EA_IN_Audit_' + Format(Date2dmy(Today, 3)) + Month1 + Dt + '.txt';
        JobQueueEntry.Reset;
        JobQueueEntry.SetRange("Object ID to Run", 50038);
        if JobQueueEntry.FindFirst then begin
            JobQueueEntry."Expected Audit File" := AuditFileName;
            JobQueueEntry.Modify;
        end;

        JobQueueEntry.Reset;
        JobQueueEntry.SetRange("Object ID to Run", 50039);
        if JobQueueEntry.FindFirst then begin
            JobQueueEntry."Expected Audit File" := AuditFileName;
            JobQueueEntry.Modify;
        end;
