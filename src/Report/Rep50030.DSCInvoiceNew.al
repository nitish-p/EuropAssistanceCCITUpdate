Report 50030 "DSC Invoice New"
{

    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/DSC Invoice New.rdl';
    UsageCategory = Lists;
    ApplicationArea = all;
            ...
            dataitem("Sales Invoice Line"; "Sales Invoice Line")
            {
                ...
                begin
                    ...
                    IF DetailedGSTLedgerEntry1.FIND('-') THEN
                        REPEAT
                            SGSTAmount := Abs(DetailedGSTLedgerEntry1."GST Amount");
                            TaxComponentSGST := DetailedGSTLedgerEntry1."GST %";
                    ...
                    IF DetailedGSTLedgerEntry2.FindSet() THEN
                        REPEAT
                            TaxComponentIGST := DetailedGSTLedgerEntry2."GST %";
                            IGSTAMT2 := Abs(DetailedGSTLedgerEntry2."GST Amount");

                            if DetailedGSTLedgerEntry2."Currency Code" = '' then
                                IGSTAmount := Abs(DetailedGSTLedgerEntry2."GST Amount")
                            else
                                IGSTAmount := IGSTAMT2 * "Sales Invoice Header"."Currency Factor"
                        UNTIL DetailedGSTLedgerEntry2.NEXT() = 0;
                    // //Totals
                    if "Sales Invoice Header"."Currency Code" <> '' then begin
                        TaxTotal += (Abs(CGSTAmount) + AbS(SGSTAmount) + Abs(IGSTAmount)) * "Sales Invoice Header"."Currency Factor";
                    end
                    else begin
                        TaxTotal += Abs(CGSTAmount) + AbS(SGSTAmount) + Abs(IGSTAmount);
                    end;
                    if "Sales Invoice Header"."Currency Code" <> '' then begin
                        IGSTAmount := IGSTAmount * "Sales Invoice Header"."Currency Factor";
                        CGSTAmount := CGSTAmount * "Sales Invoice Header"."Currency Factor";
                        SGSTAmount := SGSTAmount * "Sales Invoice Header"."Currency Factor";
                    end;
                    Total := Total + "Sales Invoice Line".Amount;
                    // Amt;
                    GrandTotal := Total + TaxTotal; //Abs(CGSTAmt) + Abs(SGSTAmt) + Abs(IGSTAmt);
                    total01 := Total + TaxTotal;
                    if "Sales Invoice Header"."Currency Code" <> '' then begin
                    ...
                    //     PostedVoucher1.FormatNoText(Notext, Round(TaxTotal), Currency);
                    ...
                    //     PostedVoucher1.InitTextVariable();
                    //     PostedVoucher1.FormatNoText(Notext, Round(TaxTotal), '');
                    // end;
